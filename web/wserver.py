...
async def lifespan(app: FastAPI):
    ...
    if Config.USER_SESSION_STRING:
        app.state.telegram_client = TgClient.mhtgClient(
            "MH-Web-User",
        ...
        app.state.telegram_client = TgClient.mhtgClient(
            f"MH-Web-Bot-{Config.BOT_TOKEN.split(':', 1)[0]}",
...
# Reverse proxy routes for stream utils when running on same port
@app.api_route(
    "/watch/{path:path}", methods=["GET", "HEAD"], response_class=HTMLResponse
)
async def proxy_watch(path: str, request: Request):
    from bot.helper.stream_utils.file_properties import get_hash_chat_id
    from bot.helper.stream_utils.render_template import render_page

    id_q = request.query_params.get("id") or path
    try:
        client = request.app.state.telegram_client
        secure_hash, chat_id, message_id = get_hash_chat_id(id_q)
        html = await render_page(chat_id, message_id, secure_hash, client=client)
        return HTMLResponse(content=html, media_type="text/html")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.api_route("/dl/{path:path}", methods=["GET", "HEAD"], response_class=HTMLResponse)
async def proxy_dl(path: str, request: Request):
    # Stream bytes from Telegram using ByteStreamer
    from bot.helper.stream_utils.file_properties import get_hash_chat_id, get_file_ids
    from bot.helper.stream_utils.custom_dl import ByteStreamer
    from starlette.responses import StreamingResponse

    bot_client = request.app.state.telegram_client
    if bot_client is None:
        raise HTTPException(status_code=503, detail="Streaming client not ready")

    id_q = request.query_params.get("id")
    if not id_q:
        # treat path as direct DDL passthrough not supported here
        raise HTTPException(status_code=404, detail="Not Found")
    try:
        secure_hash, chat_id, message_id = get_hash_chat_id(id_q)
        file_id = await get_file_ids(chat_id, message_id, client=bot_client)
        # Basic content headers
        ...
        parts = ((end // chunk_size) - (offset // chunk_size)) + 1
        streamer = ByteStreamer(bot_client)
        body = streamer.yield_file(
            file_id, offset, first_cut, last_cut, parts, chunk_size
        )
        headers = {
...
# Serve minimal JS/CSS assets for /watch relative paths (avoid 500 on /watch/js/*)
@app.get("/watch/js/{file_path:path}")
async def watch_assets(file_path: str):
    # In absence of dedicated assets, return an empty JS/CSS to satisfy requests
    if file_path.endswith((".js", ".mjs")):
        # Minimal JS to auto-play the first video element if present
        content = """
        document.addEventListener('DOMContentLoaded', function(){
          const v = document.querySelector('video');
          if (v) { v.controls = true; try { v.play(); } catch(e) {} }
        });
        """
