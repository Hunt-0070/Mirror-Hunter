...
from .. import LOGGER, user_data, cached_dict
...
async def start(_, message):
    ...
    lang = Language()
    buttons = ButtonMaker()
    buttons.url_button(lang.START_BUTTON1, "https://t.me/MirrorHunterUpdates")
    buttons.url_button(lang.START_BUTTON2, "https://t.me/MirrorHunterUpdates")
    reply_markup = buttons.build_menu(2)

    if len(message.command) > 1 and message.command[1] == "huntx":
        await delete_message(message)
    elif len(message.command) > 1:
        # Handle deep-link payloads (file store and get-all-files)
        try:
            decrypted_url = decode_slink(message.command[1])
        except ValueError as e:
            LOGGER.error(f"Failed to decode start command payload: {e}")
            # Continue with normal start flow instead of crashing
            decrypted_url = ""
        except Exception as e:
            LOGGER.error(f"Unexpected error decoding start command payload: {e}")
            decrypted_url = ""
        # Handle 'Get All Files' deep-link first to avoid matching 'file' prefix
        if decrypted_url and decrypted_url.startswith("files"):
            # Deep link to copy all files for a task to user's PM
            token = decrypted_url[5:]
            data = cached_dict.get(token)
            if not data or not isinstance(data, dict) or "entries" not in data:
                return await send_message(
                    message,
                    "<i>Link expired or invalid. Please use the latest 'Get All Files' button.</i>",
                )
            entries = data.get("entries") or []
            sent = 0
            for chat_id, msg_id in entries:
                try:
                    numeric_chat = match(r"-?\d+$", str(chat_id))
                    await TgClient.bot.copy_message(
                        chat_id=userid,
                        from_chat_id=int(chat_id) if numeric_chat else chat_id,
                    ...
                    LOGGER.error(
                        f"Failed to copy message {chat_id}/{msg_id} to {userid}: {e}"
                    )
            # One-time token cleanup
            try:
                del cached_dict[token]
            except Exception:
                pass
            return await send_message(
                message,
                f"<i>Sent {sent} file(s) to your PM.</i>",
                disable_web_page_preview=True,
            )
        elif (
            decrypted_url
            and Config.MEDIA_STORE
            and decrypted_url.startswith("file")
            and "&&" in decrypted_url[4:]
        ):
            # Remove only the 'file' prefix and copy a single message
            decrypted_payload = decrypted_url[4:]
            chat_id, msg_id = decrypted_payload.split("&&", 1)
            LOGGER.info(f"Copying message from {chat_id} & {msg_id} to {userid}")
            numeric_chat = match(r"-?\d+$", chat_id)
        ...
        # Any other type of start payload not 'file...' or 'huntx' will now fall through
        # to the generic start message or be ignored if not authorized.
