...
async def remove_from_queue(_, message):
    ...
    status = (
        msg[1] if len(msg) > 1 and msg[1] in ["fd", "fu", "fm"] else ""
    )  # Added "fm" support
...
<code>/{BotCommands.ForceStartCommand[0]}</code> fd (to remove it from download queue) or fu (to remove it from upload queue) or fm (to remove from media processing queue) or nothing to start remove it from all queues.
Also send <code>/{BotCommands.ForceStartCommand[0]} GID</code> fu|fd|fm or only gid to force start by removing the task from queue!
Examples:
<code>/{BotCommands.ForceStartCommand[1]}</code> GID fu (force upload)
<code>/{BotCommands.ForceStartCommand[1]}</code> GID fm (force media processing)
<code>/{BotCommands.ForceStartCommand[1]}</code> GID (force download, upload and media processing)
By reply to task cmd:
<code>/{BotCommands.ForceStartCommand[1]}</code> (force all queues)
<code>/{BotCommands.ForceStartCommand[1]}</code> fd (force download)
<code>/{BotCommands.ForceStartCommand[1]}</code> fm (force media processing)
"""
    ...
    async with queue_dict_lock:
        if status == "fu":
        ...
        elif status == "fm":  # New force media processing option
            if listener.mid in queued_media:
                async with media_queue_lock:
                    await start_media_from_queued(listener.mid)
                msg = "Task have been force started for media processing!"
            else:
                msg = "This task not in media processing queue!"
        else:
            listener.force_download = True
            listener.force_upload = True
            if listener.mid in queued_up:
            ...
            elif listener.mid in queued_media:
                async with media_queue_lock:
                    await start_media_from_queued(listener.mid)
                msg = "Task have been force started for media processing!"
            else:
