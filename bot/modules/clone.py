...
{limit_exceeded}""",
            ...
            if not flink:
                return
            # Call on_upload_complete with explicit keyword arguments for clarity
            ...
            if self.link.startswith("rclone_select"):
                ...
                if res[2] != 0:
                    if res[2] != -9:
                        msg = f"Error: While getting rclone stat for source path.\nPath: {remote}:{src_path}.\nStderr: {res[1][:4000]}"
            ...
            # Get post-clone stats: file count, folder count, and total size
            LOGGER.info(f"Getting stats for destination: {destination}")
            cmd1 = [
                BinConfig.RCLONE_NAME,
                "lsf",
                "--fast-list",
            ...
            if res1[2] != 0 or res2[2] != 0 or res3[2] != 0:
                ...
                msg = f"Error: Failed to get stats for cloned destination.\nPath: {destination}.\nStderr: {error[:4000]}"
                ...
                # Call on_upload_complete with explicit keyword arguments for clarity and correctness.
                # 'destination' is the full remote path, which is used to build the index link.
                await self.on_upload_complete(
                    link=flink,
                    files=files,
                    folders=folders,
                    mime_type=mime_type,
                    rclone_path=destination,
                )
        else:
            await send_message(
                self.message, COMMAND_USAGE["clone"][0], COMMAND_USAGE["clone"][1]
