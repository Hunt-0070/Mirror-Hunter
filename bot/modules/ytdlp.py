...
class MetadataProcessor:
    """Simple metadata processor for parsing and merging metadata dictionaries"""

    def parse_string(self, meta_string):
        """Parse a metadata string into a dictionary"""
        if not meta_string or not isinstance(meta_string, str):
            return {}

        metadata_dict = {}
        # Simple parsing - can be enhanced later for more complex formats
        # For now, handle simple key=value pairs separated by commas
        try:
            # Handle simple format like: title=Video Title,artist=Artist Name
            ...
            LOGGER.warning(f"Failed to parse metadata string '{meta_string}': {e}")

        return metadata_dict

    def merge_dicts(self, default_dict, override_dict):
        """Merge two dictionaries, with override_dict taking precedence"""
...
class YtDlp(TaskListener):
    def __init__(
        ...
        # Initialize metadata processor and dictionaries
        self.metadata_processor = MetadataProcessor()

        # Get user metadata settings from user_data
        user_metadata = user_data.get(self.user_id, {}).get("METADATA_SETTINGS", {})

        # Initialize metadata dictionaries from user settings or empty defaults
        self.default_metadata_dict = self._parse_metadata(
            user_metadata.get("GEN_METADATA", "")
        )
        self.audio_metadata_dict = self._parse_metadata(
            user_metadata.get("AUD_METADATA", "")
        )
        self.video_metadata_dict = self._parse_metadata(
            user_metadata.get("VID_METADATA", "")
        )
        self.subtitle_metadata_dict = self._parse_metadata(
            user_metadata.get("SUB_METADATA", "")
        )

    def _parse_metadata(self, metadata_value):
        """Helper method to parse metadata from user settings"""
        if not metadata_value or metadata_value == "Not Set":
            return {}
        return self.metadata_processor.parse_string(metadata_value)
    ...
    async def new_event(self):
        ...
        # Check for user-specific cookie file first
        user_cookie_path = f"cookies/{self.user_id}/cookies.txt"
        cookie_to_use = (
            user_cookie_path
            if not self.user_dict.get("USE_DEFAULT_COOKIE", False)
            and await aiopath.exists(user_cookie_path)
            else "cookies.txt"
        )
        LOGGER.info(
