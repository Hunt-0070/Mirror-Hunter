...
from ..core.owner_utils import is_owner
...
async def get_buttons(key=None, edit_type=None, edit_mode=False, user_id=None):
    buttons = ButtonMaker()
    if key is None:
        ...
        auto_resume_status = "Enabled" if Config.TASK_AUTO_RESUME else "Disabled"
        buttons.data_button(
            f"Task Auto Resume: {auto_resume_status}", "botset toggle_auto_resume"
        )
        auto_cancel_status = (
            "Enabled" if Config.AUTO_CANCEL_STALLED_TASKS else "Disabled"
        )
        buttons.data_button(
            f"Auto-Cancel Stalled Tasks: {auto_cancel_status}",
            "botset toggle_auto_cancel",
        )
        ...
        for k in list(conf_dict.keys())[start : 10 + start]:
            if k in ["UPSTREAM_REPO", "UPSTREAM_BRANCH"]:
                if not is_owner(user_id):
                    continue
            elif k in SENSITIVE_KEYS:
                if not is_owner(user_id):
                    continue
            # Original condition for DATABASE_URL, kept for safety, though SENSITIVE_KEYS check should cover it.
            if k == "DATABASE_URL" and state != "view" and not is_owner(user_id):
                continue
...
async def update_buttons(
    message, key=None, edit_type=None, edit_mode=False, user_id=None
):
    if user_id is None:
        user_id = message.from_user.id if message.from_user else None
    msg, button = await get_buttons(key, edit_type, edit_mode, user_id)
...
async def edit_variable(_, message, pre_message, key):
    ...
    # Only restrict sensitive variables to owner
    if key in SENSITIVE_KEYS and not is_owner(message.from_user.id):
        await send_message(
            message, f"Only the owner can edit sensitive variable '{key}'."
        )
        return
    ...
    if value.lower() == "true":
        ...
        value = False
    elif key == "INCOMPLETE_TASK_NOTIFIER" and Config.DATABASE_URL:
        ...
        value = int(value)
    elif key == "AUTO_CANCEL_STALLED_TASKS":
        value = str(value).lower() == "true"
        # Restart the stalled task monitor with new setting
        from ..helper.ext_utils.stalled_task_monitor import stalled_task_monitor

        if value:
            await stalled_task_monitor.start_monitoring()
        else:
            await stalled_task_monitor.stop_monitoring()
    elif key == "STALLED_TASK_TIMEOUT":
        value = int(value)
        if value < 1:
            value = 1  # Minimum 1 minute
        elif value > 1440:
            value = 1440  # Maximum 24 hours
        ...
        excluded_extensions.extend(["aria2", "!qB", "ass"])
        for x in fx:
            x = x.lstrip(".")
    ...
    user_id = message.from_user.id if message.from_user else None
    await update_buttons(pre_message, "var", user_id=user_id)
...
async def edit_aria(_, message, pre_message, key):
    ...
    user_id = message.from_user.id if message.from_user else None
    await update_buttons(pre_message, "aria", user_id=user_id)
    await delete_message(message)
...
async def edit_qbit(_, message, pre_message, key):
    ...
    user_id = message.from_user.id if message.from_user else None
    await update_buttons(pre_message, "qbit", user_id=user_id)
    await delete_message(message)
...
async def edit_nzb(_, message, pre_message, key):
    ...
    user_id = message.from_user.id if message.from_user else None
    await update_buttons(pre_message, "nzb", user_id=user_id)
    await delete_message(message)
    ...
    if key == "newser":
        if value.startswith("{") and value.endswith("}"):
            try:
                ...
                user_id = message.from_user.id if message.from_user else None
                await update_buttons(pre_message, "nzbserver", user_id=user_id)
                return
        ...
        user_id = message.from_user.id if message.from_user else None
        await update_buttons(pre_message, f"nzbser{index}", user_id=user_id)
    await delete_message(message)
...
async def update_private_file(_, message, pre_message, key, new_file=False):
    ...
    if not message.media and (raw_file_name := message.text):
        # Determine file_name for deletion or new file creation case
        if new_file:
            # For new file creation, file_name is part of the message text
            file_name = raw_file_name.split("\n", 1)[0].strip()
        else:
            # For deletion, the whole message is the file_name
            file_name = raw_file_name.strip()

        if file_name == "config.py" and not is_owner(user_id):
            await send_message(
                message, "Only the Owner is allowed to modify <code>config.py</code>."
            )
            await update_buttons(
                pre_message, key
            )  # Update buttons to reflect no change
            return

        if new_file:
            # Content is also part of the message text for new file
            _, content = raw_file_name.split("\n", 1)
            async with aiopen(file_name, "w") as f:
                await f.write(content.strip())
        else:  # This is the deletion case for text input
            if (
                await aiopath.isfile(file_name) and file_name != "config.py"
            ):  # config.py deletion handled by owner check
                await remove(file_name)
            elif file_name == "config.py" and is_owner(
                user_id
            ):  # Explicitly allow owner to delete config.py via text
                if await aiopath.isfile(file_name):
                    await remove(file_name)

            if (
                file_name == "accounts.zip"
            ):  # This part is for post-deletion/modification handling
        ...
        if file_name == "config.py" and not is_owner(user_id):
            await send_message(
                message, "Only the Owner is allowed to upload <code>config.py</code>."
            )
            await update_buttons(
                pre_message, key
            )  # Update buttons to reflect no change
            # We might want to delete the downloaded file if it was saved temporarily by message.download
            # However, the current flow downloads after this check. Let's ensure it's handled or pre-checked.
            # For now, just returning and deleting the trigger message.
            await delete_message(message)
            return
        ...
        if await aiopath.exists(
            fpath
        ):  # Potentially delete existing before download, esp if it's config.py by owner
            if file_name == "config.py" and is_owner(user_id):
                await remove(fpath)  # Owner replacing config.py
            elif file_name != "config.py":
                await remove(fpath)  # Other files
            # If it's config.py and not owner, we would have returned already.
...
async def edit_bot_settings(client, query):
    ...
    handler_dict[message.chat.id] = False
    # Restrict sensitive actions to owner only
    if (
        data[1] in ["botvar", "resetvar"]
        and data[2] in SENSITIVE_KEYS
        and not is_owner(user_id)
    ):
        await query.answer(
            "Only the owner can view or edit this variable.", show_alert=True
        )
        return
    if data[1] == "close":
        await query.answer()
        ...
        await query.answer()
        globals()["start"] = 0
        await update_buttons(message, None, user_id=user_id)
    elif data[1] == "syncjd":
    ...
    elif data[1] == "toggle_auto_cancel":
        await query.answer()
        current_value = Config.AUTO_CANCEL_STALLED_TASKS
        new_value = not current_value
        Config.set("AUTO_CANCEL_STALLED_TASKS", new_value)
        await database.update_config({"AUTO_CANCEL_STALLED_TASKS": new_value})
        ...
        if new_value:
            await stalled_task_monitor.start_monitoring()
            await edit_message(message, "Auto-Cancel Stalled Tasks: Enabled")
        else:
            await stalled_task_monitor.stop_monitoring()
            await edit_message(message, "Auto-Cancel Stalled Tasks: Disabled")
        ...
        if data[1] == "nzbserver":
            globals()["start"] = 0
        await query.answer()
        await update_buttons(message, data[1], user_id=user_id)
        ...
        if data[2] in DEFAULT_VALUES:
            ...
            excluded_extensions.clear()
            excluded_extensions.extend(["aria2", "!qB", "ass"])
            await edit_message(message, "Excluded Extensions restored to defaults.")
        elif data[2] == "TASK_AUTO_RESUME":
            value = DEFAULT_VALUES[data[2]]
        elif data[2] == "TORRENT_TIMEOUT":
            await TorrentManager.change_aria2_option("bt-stop-timeout", "0")
            await database.update_aria2("bt-stop-timeout", "0")
        elif data[2] == "AUTO_CANCEL_STALLED_TASKS":
            value = DEFAULT_VALUES[data[2]]
            # Restart the stalled task monitor with new setting
            from ..helper.ext_utils.stalled_task_monitor import stalled_task_monitor
            ...
            if value:
                ...
                await stalled_task_monitor.stop_monitoring()
        elif data[2] == "STALLED_TASK_TIMEOUT":
            value = DEFAULT_VALUES[data[2]]
            if value < 1:
                value = 1  # Minimum 1 minute
            elif value > 1440:
        ...
        await update_buttons(message, "var", user_id=user_id)
        ...
        await update_buttons(message, "nzb", user_id=user_id)
        await database.update_nzb_config()
    elif data[1] == "syncnzb":
        ...
        await update_buttons(message, "aria", user_id=user_id)
        ...
        qbit_options[data[2]] = ""
        await update_buttons(message, "qbit", user_id=user_id)
        ...
        await update_buttons(message, "nzbserver", user_id=user_id)
        await database.update_config({"USENET_SERVERS": Config.USENET_SERVERS})
    elif data[1] == "private":
        await query.answer()
        if data[2] in ("open", "stop"):
            ...
            rfunc = partial(update_buttons, message, data[1], user_id=user_id)
            await event_handler(client, query, pfunc, rfunc, True)
    elif data[1] == "botvar" and state == "edit":
        await query.answer()
        await update_buttons(message, data[2], data[1])
        pfunc = partial(edit_variable, pre_message=message, key=data[2])
        rfunc = partial(update_buttons, message, "var", user_id=user_id)
        await event_handler(client, query, pfunc, rfunc)
    elif data[1] == "botvar" and state == "view":
        if data[2] in SENSITIVE_KEYS and not is_owner(user_id):
            await query.answer(
                "Only the owner can view this variable.", show_alert=True
            )
            return
        ...
        rfunc = partial(update_buttons, message, "aria", user_id=user_id)
        await event_handler(client, query, pfunc, rfunc)
        ...
        rfunc = partial(update_buttons, message, "qbit", user_id=user_id)
        await event_handler(client, query, pfunc, rfunc)
        ...
        rfunc = partial(update_buttons, message, "nzb", user_id=user_id)
        await event_handler(client, query, pfunc, rfunc)
        ...
        await query.answer()
        globals()["state"] = "edit"
        await update_buttons(message, data[2], user_id=user_id)
    elif data[1] == "view":
        await query.answer()
        globals()["state"] = "view"
        await update_buttons(message, data[2], user_id=user_id)
