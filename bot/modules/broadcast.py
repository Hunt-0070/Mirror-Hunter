...
async def broadcast_start_message(start_msg: str):
    """Broadcast a start message to all PM users (used on bot restart)."""
    from ..helper.telegram_helper.message_utils import send_message
    from ..helper.ext_utils.db_handler import database
    from .. import LOGGER

    uids = await database.get_pm_uids()
    sent, failed = 0, 0
    for uid in uids:
        try:
            await send_message(uid, start_msg)
            sent += 1
        except Exception as e:
            LOGGER.warning(f"Failed to send start message to {uid}: {e}")
            failed += 1
    LOGGER.info(f"Broadcasted start message to {sent} users, failed for {failed}.")
