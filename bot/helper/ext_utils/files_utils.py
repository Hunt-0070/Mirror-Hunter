...
from bot import LOGGER  # user_data is available at bot level
from bot.core.config_manager import Config  # Corrected import for Config

# Added missing utility import for readable file sizes
...
async def clean_download(opath):
    if await aiopath.exists(opath):
        try:
            ...
            LOGGER.error(f"[CLEAN] Exception in clean_download for {opath}: {str(e)}")
    else:
        pass  # Path doesn't exist, nothing to do or log for info level
...
async def get_path_sizee(path: str):
    ...
    for root, _, files in await sync_to_async(walk, path):
        for f in files:
            abs_path = ospath.join(root, f)
            if await aiopath.exists(abs_path):
                total_size += await aiopath.getsize(abs_path)
    return total_size


def ffmpeg_parse(name: str):
...
async def move_and_merge(source, destination, mid):
    ...
    try:
        if not await aiopath.exists(source):
            LOGGER.error(f"move_and_merge: Source path does not exist: {source}")
            return False
            ...
            if await aiopath.isdir(src_path):
                if await aiopath.exists(dest_path):
                    ...
                    try:
                        ...
                        LOGGER.error(
                            f"move_and_merge: Error moving directory {src_path} to {dest_path}: {e}"
                        )
                        continue
            else:
                ...
                try:
                    ...
                    LOGGER.error(
                        f"move_and_merge: Error moving file {src_path} to {dest_path}: {e}"
                    )
                    continue
        return True
    except Exception as e:
        LOGGER.error(
            f"move_and_merge: Critical error during merge from {source} to {destination}: {e}"
        )
        return False
...
async def join_files(opath):
    try:
        if not await aiopath.exists(opath):
            LOGGER.error(f"join_files: Path does not exist: {opath}")
            return []
            ...
            if re_search(r"\.0+2$", file_) and await sync_to_async(
                ...
                try:
                    ...
                    if code != 0:
                        ...
                        LOGGER.info(f"Successfully joined: {final_name}")
                except Exception as e:
                    LOGGER.error(f"Error joining {final_name}: {e}")
                    if await aiopath.isfile(fpath):
                        await remove(fpath)
                    continue
            await sleep(0.2)  # Reduced CPU usage from 0.05 to 0.2 seconds

        for file_ in files:
            if re_search(r"\.7z\.0+2$", file_):
                final_name = file_.rsplit(".", 2)[0]
                fpath = f"{opath}/{final_name}"
                cmd = f'7z x "{fpath}.7z.001" -o"{opath}" -y'
                try:
                    _, stderr, code = await cmd_exec(cmd, True)
                    if code != 0:
                        LOGGER.error(
                            f"Failed to extract {final_name}.7z.001, stderr: {stderr}"
                        )
                    else:
                        results.append(final_name)
                        LOGGER.info(f"Successfully extracted: {final_name}")
                except Exception as e:
                    LOGGER.error(f"Error extracting {final_name}: {e}")
                    continue
            await sleep(0.2)  # Reduced CPU usage from 0.05 to 0.2 seconds

        if not exists:
            LOGGER.warning("join_files: No files to join!")
        elif results:
            LOGGER.info("join_files: Join Completed!")
            try:
                for res in results:
                    for file_ in files:
                        if re_search(rf"{escape(res)}\.0[0-9]+$", file_):
                            await remove(f"{opath}/{file_}")
                            LOGGER.debug(f"Removed joined part: {file_}")
            except Exception as e:
                LOGGER.error(f"join_files: Error cleaning up joined parts: {e}")

        return results
    except Exception as e:
        LOGGER.error(f"join_files: Critical error in {opath}: {e}")
        return []
...
async def check_font_zip_structure(zip_path, temp_extract_dir=None):
    """
    Check if a ZIP file contains fonts suitable for hardsub by examining its structure.

    Args:
        zip_path (str): Path to the ZIP file to examine
        temp_extract_dir (str, optional): Temporary directory for extraction.
    ...
    result = {
        ...
        "total_fonts": 0,
        "main_dir_fonts": 0,
        "structure_info": {"has_main_dir": False, "directories": [], "total_files": 0},
    }

    if not await aiopath.exists(zip_path):
    ...
    if temp_extract_dir is None:
        temp_extract_dir = tempfile.mkdtemp(prefix="font_zip_check_")
        cleanup_temp = True
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            result["structure_info"]["total_files"] = len(file_list)
            ...
            for file_path in file_list:
                ...
                if (
                    ...
                    and file_path.lower().endswith(font_extensions)
                    and not file_path.startswith("__MACOSX")
                    and "__MACOSX" not in file_path
                    and ".." not in file_path
                ):  # Security check
                    all_font_files.append(file_path)

                    # Check if font is in main directory (no subdirectories)
                    if "/" not in file_path or file_path.count("/") == 0:
                        main_dir_fonts.append(file_path)

            result["structure_info"]["directories"] = sorted(list(directories))
            result["structure_info"]["has_main_dir"] = len(directories) > 0
            result["font_files"] = all_font_files
            result["total_fonts"] = len(all_font_files)
            result["main_dir_fonts"] = len(main_dir_fonts)

            # Determine if this is a valid font ZIP
            # Consider it valid if it has any font files
            result["is_font_zip"] = result["total_fonts"] > 0

            if result["is_font_zip"]:
                LOGGER.info(
                    f"Font ZIP validated: {zip_path} - "
                    f"{result['total_fonts']} fonts found "
                    f"({result['main_dir_fonts']} in main directory)"
                )
            else:
                LOGGER.warning(f"No valid font files found in ZIP: {zip_path}")

    except zipfile.BadZipFile:
        LOGGER.error(f"Invalid ZIP file: {zip_path}")
    except Exception as e:
        LOGGER.error(f"Error checking font ZIP structure: {e}")
    finally:
        # Cleanup temporary directory if we created it
        if cleanup_temp and await aiopath.exists(temp_extract_dir):
            try:
                await aiormtree(temp_extract_dir, ignore_errors=True)
            except Exception as e:
                LOGGER.warning(
                    f"Failed to cleanup temp directory {temp_extract_dir}: {e}"
                )
...
class SevenZ:
    ...
    async def _sevenz_progress(self):
        ...
        subname_pattern = re.compile(r"-\s+(.*)")
        while not (
            ...
            if subname_match := subname_pattern.search(line):
                self._listener.subname = subname_match.group(1)
            elif match := re_search(pattern, line):
    ...
    async def extract(self, f_path, t_path, pswd):
        threads = (
            getattr(Config, "EXTRACT_THREADS", 2) or 2
        )  # Increased default from 1 to 2
        cmd = [
            "7z",
            "x",
            f"-p{pswd}",
            ...
            f"-o{t_path}",
            "-aot",
            #   f"-mmt={threads}",  # Enable multi-threading for faster extraction
            "-xr!@PaxHeader",
            "-bsp1",
            "-bse1",
            "-bb3",
