...
class TelegramDownloadHelper:
    ...
    async def _on_download_start(self, file_id, from_queue):
        ...
        if not from_queue:
            ...
            if self._listener.multi <= 1:
                if (
                    self._listener.message.chat.type
                    == self._listener.message.chat.type.PRIVATE
                ):
                    await send_status_message(
                        self._listener.message, self._listener.user_id
                    )
                else:
    ...
    async def add_download(self, reply_to, path, session):
        ...
        if Config.BIN_CHANNEL:
            message = await reply_to.copy(int(Config.BIN_CHANNEL))
        else:
            message = reply_to
        if self.session == "user":
            user_client = await UserSessionManager.get_user_session(
