...
class YoutubeDLHelper:
    def __init__(self, listener):
        ...
        user_cookies_path = (
            f"cookies/{getattr(listener, 'user_id', None)}/cookies.txt"
            if getattr(listener, "user_id", None)
            else None
        )
        cookies_file = (
            user_cookies_path
    ...
    def _set_options(self, options):
        for key, value in options.items():
            if key == "postprocessors":
                ...
                # Don't override user-specific cookie file if it exists
                user_id = getattr(self._listener, "user_id", None)
                if user_id:
                    import os

                    user_cookie_path = f"cookies/{user_id}/cookies.txt"
                    if os.path.exists(user_cookie_path):
                        # Keep the user-specific cookie file, don't override
                        continue
                self.opts[key] = value
            else:
                self.opts[key] = value
