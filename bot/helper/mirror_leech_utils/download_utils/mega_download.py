...
async def add_mega_download(listener, path):
    async_api = AsyncMega()
    async_api.api = api = MegaApi(None, None, None, "Mirror-Hunter")
    folder_api = None
    ...
    if get_mega_link_type(listener.link) == "file":
        ...
        async_api.folder_api = folder_api = MegaApi(None, None, None, "Mirror-Hunter")
        ...
        if node:
            ...
            LOGGER.error(
                "Failed to authorize node. It might be an invalid link or an issue with Mega API."
            )
            await listener.on_download_error(
                "Failed to authorize Mega node. Invalid link or API issue."
            )
            await async_api.logout()
            if folder_api:  # ensure folder_api is logged out if it exists
