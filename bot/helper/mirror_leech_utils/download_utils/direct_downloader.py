...
from .direct_link_generator import safe_int_size
...
async def add_direct_download(listener, path):
    ...
    # Calculate total size if not set or if it's 0
    total_size = details.get("total_size", 0)
    if not total_size or total_size <= 0:
        # Recalculate total size from contents
        calculated_size = 0
        for item in contents:
            size = item.get("size")
            calculated_size += safe_int_size(size)
