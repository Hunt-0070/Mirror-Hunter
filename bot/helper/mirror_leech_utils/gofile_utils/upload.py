...
class GoFileUpload:
    ...
    async def __resp_handler(self, response):
        if response is None:
            raise Exception("No response received from GoFile API")
    ...
    async def __getContent(self, contentId):
        if self.token is None:
            raise Exception("GoFile API token not found!")

        async with ClientSession() as session:
            async with session.get(
                url=f"{self.api_url}contents/{contentId}?token={self.token}"
            ) as resp:
                return await self.__resp_handler(await resp.json())
    ...
    async def _upload_dir(self, input_directory, parent_folder_id=None):
        if parent_folder_id is None:
            ...
            if self.folder_id:
                try:
                    folder_details = await self.__getContent(self.folder_id)
                    if folder_details.get("type") == "folder":
                        folder_to_use = {
                            "id": self.folder_id,
                            "code": folder_details["code"],
                        }
                    else:
                        LOGGER.warning(
                            f"Provided GoFile ID {self.folder_id} is not a folder. Creating a new one."
                        )
                except Exception as e:
                    LOGGER.warning(
                        f"GoFile folder ID {self.folder_id} is invalid: {e}. Creating a new one."
                    )

            if folder_to_use:
                parent_folder_id = folder_to_use["id"]
                main_folder_code = folder_to_use["code"]
            else:
        ...
        for root, dirs, files in await sync_to_async(oswalk, input_directory):
            ...
            if rel_path != ".":
                ...
                await self.__setOptions(
                    contentId=curr_folder_data["id"],
                    option="public",
                    value="true",
                )
                folder_ids[rel_path] = curr_folder_data["id"]
                current_folder_id = curr_folder_data["id"]
