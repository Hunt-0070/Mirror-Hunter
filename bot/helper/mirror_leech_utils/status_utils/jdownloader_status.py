...
class JDownloaderStatus:
    ...
    async def cancel_task(self):
        LOGGER.info(f"[CANCEL] JDownloaderStatus.cancel_task START for {self.name()}")
        self.listener.is_cancelled = True
        LOGGER.info(f"[CANCEL] JDownloaderStatus: set is_cancelled for {self.name()}")
        await jdownloader.device.downloads.remove_links(
            package_ids=jd_downloads[self._gid]["ids"]
        )
        LOGGER.info(
            f"[CANCEL] JDownloaderStatus: remove_links complete for {self.name()}"
        )
        async with jd_listener_lock:
            del jd_downloads[self._gid]
        LOGGER.info(
            f"[CANCEL] JDownloaderStatus: del jd_downloads entry for {self.name()}"
        )
        await self.listener.on_download_error("Cancelled by user!")
        LOGGER.info(f"[CANCEL] JDownloaderStatus.cancel_task END for {self.name()}")
