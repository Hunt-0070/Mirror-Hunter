...
class GoogleDriveClone(GoogleDriveHelper):
    ...
    def clone(self):
        ...
        try:
            ...
            # Use correct key and fallbacks for Index URL
            index_url = (
                self.listener.user_dict.get("INDEX_URL")
                or getattr(self.listener, "INDEX_URL", None)
                or getattr(Config, "INDEX_URL", "")
            )
            return (
                durl,
                mime_type,
            ...
            if "User rate limit exceeded" in err:
                ...
                if not self.alt_auth and self.use_sa:
                    ...
                    return self.clone()
                msg = "File not found."
            elif "before_request" in err or "NoneType" in err:
                msg = "This Google Drive file/folder is private and cannot be accessed. Please make sure the file is public or you have proper permissions."
            elif (
                "403" in err
                or "Forbidden" in err
                or "insufficientFilePermissions" in err
            ):
                msg = "Access denied. This Google Drive file/folder is private. Please make the file public or check your permissions."
            else:
                msg = f"Error.\n{err}"
