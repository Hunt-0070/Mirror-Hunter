...
class DirectListener:
    ...
    async def cancel_task(self):
        LOGGER.info(
            f"[CANCEL] DirectListener.cancel_task START for {self.listener.name}"
        )
        self.listener.is_cancelled = True
        LOGGER.info(
            f"[CANCEL] DirectListener: set is_cancelled for {self.listener.name}"
        )
        await self.listener.on_download_error("Download Cancelled by User!")
        LOGGER.info(
            f"[CANCEL] DirectListener: on_download_error complete for {self.listener.name}"
        )
        if self.download_task:
            await TorrentManager.aria2_remove(self.download_task)
            LOGGER.info(
                f"[CANCEL] DirectListener: aria2_remove complete for {self.listener.name}"
            )
        LOGGER.info(f"[CANCEL] DirectListener.cancel_task END for {self.listener.name}")
